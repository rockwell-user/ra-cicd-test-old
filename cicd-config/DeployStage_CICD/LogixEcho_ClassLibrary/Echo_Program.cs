...
namespace LogixEcho_ClassLibrary
{
    ...
    {
        /// <summary>
        /// Script that sets up an emulated controller to be the deployment target in the CI/CD deployment stage.<br/>
        ...
        /// </summary>
        /// <param name="acdFilePath">The file path pointing to the ACD project used for deployment.</param>
        /// <returns>A string containing the communication path of the emulated controller that the ACD project file will go online with during deployment.</returns>
        public static async Task<string> Main(string acdFilePath)
        {
            var serviceClient = ClientFactory.GetServiceApiClientV2("CLIENT_DeployStage_CICDExample");
            serviceClient.Culture = new CultureInfo("en-US");

            // Check if an emulated controller CICD_deploy exists within an emulated chassis CICDdeploy_chassis. If not, run through the if statement contents to create one.
            if (CheckCurrentChassis_Sync("CICDdeploy_chassis", "CICD_deploy", serviceClient) == false)
            {
                // Set up emulated chassis information.
                var chassisUpdate = new ChassisUpdate
                {
                    Name = "CICDdeploy_chassis",
